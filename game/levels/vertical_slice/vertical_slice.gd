...
##
## In dieser Ausbaustufe (P0-02) gibt es noch keine Teilnehmer (Player, Creature,
## Interactables, Puzzles); sie bleiben ausdrücklich optional und werden erst mit
## ihrem jeweiligen Task angebunden. Keine Sonderzweige für Testwelten.

## Name der Welt für die Anzeige in der UI; keine Gameplaywahrheit.
@export var display_name: String = "Welt"

## Einzige aktive Weltkamera. In der Sandbox die feste Testkamera, im
## Vertical Slice später die Player-Kamera. Pflichtreferenz.
@export var world_camera: Camera3D

var _gameplay_active: bool = false


## Prüft die benötigten Referenzen und macht die Weltkamera zur aktiven Kamera.
## Gibt false zurück, wenn die Welt nicht freigegeben werden darf; Main baut
## sie dann kontrolliert wieder ab. Gameplay bleibt nach dem Aufruf gesperrt.
...
## Einziger Freigabeauftrag von Main. Spätere Teilnehmer (Player, Creature,
## Interactables, Trigger) erhalten die Freigabe von hier aus weitergereicht.
